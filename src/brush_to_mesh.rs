...
fn triangulate_face(normal: Vec3, boundary: &[Vec3], target_spacing: f32) -> (Vec<Vec3>, Vec<[usize; 3]>) {
    ...
    let mut corners_2d: Vec<(f32, f32)> = boundary.iter().map(|&p| project(p)).collect();
    let centroid = {
        let sum = corners_2d.iter().fold((0.0, 0.0), |acc, p| (acc.0 + p.0, acc.1 + p.1));
        (sum.0 / corners_2d.len() as f32, sum.1 / corners_2d.len() as f32)
    };
    corners_2d.sort_by(|a, b| {
        let ta = (a.1 - centroid.1).atan2(a.0 - centroid.0);
        let tb = (b.1 - centroid.1).atan2(b.0 - centroid.0);
        ta.partial_cmp(&tb).unwrap()
    });
    let boundary_2d = subdivide_boundary(&corners_2d, target_spacing);
...
fn subdivide_boundary(corners_ccw: &[(f32, f32)], spacing: f32) -> Vec<(f32, f32)> {
    if spacing <= 0.0 {
...
mod tests {
    ...
    fn subdivide_boundary_adds_evenly_spaced_points_along_each_edge() {
        ...
        let subdivided = subdivide_boundary(&corners, 3.0);
    ...
    fn subdivide_boundary_is_a_no_op_for_non_positive_spacing() {
        let corners = vec![(0.0, 0.0), (10.0, 0.0), (10.0, 10.0), (0.0, 10.0)];
        assert_eq!(subdivide_boundary(&corners, 0.0), corners);
    }
